...
async def service_handler(icap_request: ICAPRequest) -> ContentAdaptationResponse:
    """
    Handle incoming ICAP requests that are available for content adaption.

    :param icap_request: An ICAP request available for content adaption.
    ...
    match method := icap_request.request_line.method:
        ...
        case ICAPMethod.REQMOD:
            icap_response_code = 200
        case _:
            raise ValueError(f'Unexpected ICAP method: {method}')
